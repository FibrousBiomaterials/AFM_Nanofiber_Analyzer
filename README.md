...
| `guis/GUI06_ML_Model_Compare.py` | ML Model Compare | Apply a trained model to `.b2z` bundles and compare it against the classical result. Binarization and background-mask models are scored mask-to-mask with Dice / IoU / agreement; a background-surface model is scored in nanometers against the surface the pipeline subtracted. Use it to judge whether a model is worth integrating before adding it to the preprocessing pipeline. |
