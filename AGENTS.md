...
| `connect_selection.py` | `CONNECT_SUFFIX`, `CONNECT_FORMAT`, `CONNECT_VERSION`, `CONNECT_PARAM_FIELDS`, `ConnectSettings`, `connect_path_for`, `connect_state_key`, `params_to_dict`, `params_from_dict`, `load_connect_settings`, `save_connect_settings` | Fiber-connection settings recorded in `<stem>_connect.json` beside the bundle, written by GUI04's "除外・連結を保存" — which writes this file and the exclusion sidecar in one press, deliberately offering no way to write only one, because `curate_fibers` reads them together and a pair recorded at different moments describes a population that was never on screen (measured: 29 aggregated fibrils against 56 displayed fragments, unsignalled) — and applied by `measure.collect_fiber_stats(apply_connection=True)` and its three sibling collectors. It is the sibling of `fiber_selection.py` but **not the same kind of record**: an exclusion is an irreproducible human judgement and must be stored as data, while connection is a deterministic function of the bundle and `ConnectParams`, so only `enabled` plus the six thresholds are stored and the connector is re-run at measurement time. A stored list of joins would be a cache that can silently disagree with the bundle beside it. `CONNECT_PARAM_FIELDS` names are serialized verbatim and are frozen for the same reason `ProcParams` field names are (§8.1); a missing one falls back to its dataclass default. The `links` key is **reserved for manual per-pair connection decisions and written empty from version 1**, so adding them later needs no format break: a version 1 reader refuses a file that carries entries, and the writer that starts filling them must raise `CONNECT_VERSION` — a reader that silently ignored them would measure a population the user never chose. A file whose `version` exceeds `CONNECT_VERSION` is refused rather than parsed for the keys this version happens to understand, and a malformed file raises instead of reading as "not connected", because falling back silently would measure fragments while the user believes whole fibrils were measured. Unlike `save_exclusions`, turning the feature off writes `enabled: false` instead of deleting the file: a stale exclusion file would restore fibers the user rejected, whereas "off" and "no file" measure identically here, and keeping the file preserves both the tuned thresholds and the fact that not connecting was a decision. `load_connect_settings` returns `None` for a missing file, which is deliberately distinct from a stored `enabled: false` — GUI03 counts how many bundles carry a decision. Depends only on the standard library and `fiber_connector.ConnectParams`. |
| `fiber.py` | `Fiber` | Immutable dataclass holding height, length, kink points, and endpoints per fiber. |
| `fiber_connector.py` | `ConnectParams`, `connect_fiber_fragments`, `connection_candidate_flags`, `filter_fibers_by_height`, `angle_between_three_points` | `connection_candidate_flags` answers "could the connector extend this fragment?" as a **standalone, order-independent predicate over the original fragments** — deliberately not a record of what `connect_fiber_fragments` did, because that function consumes fragments as it grows, so which joins happen depends on the visiting order and a predicate used to judge a fiber cannot depend on it. It applies the same distance, angle, and height gates, evaluated on each fragment's own end, look-back direction, and median height, and flags **both** fragments of a joinable pair of ends (the connector's gate is written from the growing side and is not symmetric, so one orientation alone under-reports). Measured against what the connector actually joined on two real scans, it agreed on 145 of 146 extended fragments. Used by `measure.isolated_fiber_flags`, and **only in conjunction with the branch-point and frame tests** — alone it is about five times looser, because "no candidate" conflates "complete" with "the connector could not tell", which is the common case in a dense tangle. Reconnects skeleton fragments split at crossings/branches into whole fibrils by local position/direction/height continuity, rebuilding one `Fiber` per fibril (features recomputed via `KinkDetector`). Used by GUI04's optional fiber-connection mode via `measure_bundle(connect_fibers=True)`. Sequential (order-dependent), so not parallelized. `filter_fibers_by_height` implements GUI04's "connect, then filter" order: it slices each already-built fibril by its own `Fiber.height` profile (bridge heights included, so an in-band bridge keeps the fibril joined) instead of masking the raw skeleton like `FiberTrackingImage.specific_height_fibers`. |
