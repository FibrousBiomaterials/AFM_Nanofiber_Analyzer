...
def test_historical_calibrator_name_still_importable():
    """The pre-rename import path and class name resolve to the same class.

    Existing scripts and saved settings reference `BG_Calibrator_shimadzu`,
    so the compatibility shim must keep that name working.
    既存のスクリプトや保存済み設定は `BG_Calibrator_shimadzu` を参照する
    ため、互換シムが旧名を機能させ続けなければならない。
    """
    from lib.bg_calibrator_shimadzu import BG_Calibrator_shimadzu
    assert BG_Calibrator_shimadzu is BGCalibrator
