...
This guard is local-only: it is skipped on CI (``CI`` / ``GITHUB_ACTIONS``)
because the exact-byte baseline reproduces only on the machine that generated
it. The portable, tolerant pipeline check for CI is ``test_integration.py``.
本ガードはローカル専用で、CI（``CI`` / ``GITHUB_ACTIONS``）ではスキップする。
厳密バイトのベースラインは生成した環境でしか再現しないためである。CI 向けの
移植性ある許容パイプライン検査は ``test_integration.py`` が担う。
...
# Exact-byte SHA-256 baselines are environment-specific: floating-point results
# from scipy / lmfit / OpenCV and the underlying BLAS/LAPACK differ in their
# lowest bits across OS, CPU, and dependency versions, so the golden file only
# reproduces on the machine that generated it. Use this as a local before/after
# guard for refactors, and skip it on CI, where the environment never matches
# the baseline and an exact-hash gate would fail for non-code reasons.
# 完全一致 SHA-256 ベースラインは環境依存である。scipy / lmfit / OpenCV や
# その下層の BLAS/LAPACK の浮動小数点結果は OS・CPU・依存バージョンで最下位
# ビットが変わるため、golden は生成した環境でしか再現しない。リファクタ前後
# のローカル確認用ガードとして使い、ベースラインと環境が一致せずコード以外の
# 理由で失敗する CI ではスキップする。
