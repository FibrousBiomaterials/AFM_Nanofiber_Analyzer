...
バッチは自動選択のみ）。長さ単位の高さチャンネルは Gwyddion の SI 単位から nm へ、
走査範囲はチャンネルの `xreal`/`yreal` から µm へ換算します。

長さ以外のチャンネルを明示選択した場合、その値は元単位のまま返されます。
GUI02 の高さラベルや前処理パイプラインは nm の高さ画像を前提とするため、
位相・電流などのチャンネルは表示／解析対象として適切ではありません。
...
`lib/imp_tools.py` の `branchedPoints` / `endPoints` は、元のラボコード
（mahotas ベース）の hit-or-miss テンプレートを OpenCV `MORPH_HITMISS` へ
バイト単位で一致するよう移植したものです。テンプレートとその回転順は
`_build_branch_patterns` / `_build_end_patterns` で固定されており、変更すると
分岐点検出が変わり、枝刈り・ファイバー分割・長さ・キンク統計、および
`tests/test_strict_regression.py` の golden 出力に波及します。

過去に `diagonal_ybranch = [[0,1,2],[1,1,2],[2,2,1]]` には「ワイルドカードが
多くパターンをカバーしすぎではないか」という `TODO(review)` が付いていました。
同梱テストスキャン全件で固有寄与を実測した結果、次が確認できたためフラグを
解消し、設計意図として確定させています。

- このカーネル（4 回転）は全分岐点の**約半数**を単独で供給する（最終骨格で
  249/522、初期骨格で 626/1277）。冗長ではなく load-bearing。
- 固有検出画素のほぼ全ては他カーネルの検出点に隣接しない独立検出であり、
  その**全件が骨格近傍次数 ≥3 の真の局所分岐**（最終骨格で次数3が 94.4%、
  次数4が 5.6%、次数2 = 直線パス上の偽検出は 0 件）。
- 緩さの実体は「軸平行カーネルが取りこぼす斜め方向の Y 分岐を拾う」ことで
  あり、偽分岐の生成ではない。

したがってこのカーネルの削除・厳格化は単なる整理ではなく、検出を半減させる
**意図的なアルゴリズム変更**です。実施する場合はドメイン側の妥当性確認と
golden 出力の再生成（`tests/test_strict_regression.py`）をセットで行ってください。
