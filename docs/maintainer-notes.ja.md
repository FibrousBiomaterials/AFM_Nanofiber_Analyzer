...
起動高速化のため、`Main.py` は `PRELOAD_LIBS` に列挙した重いライブラリや
主要プロジェクトモジュールを事前 import します。PyInstaller の frozen build
では `-c` や `-m` による再起動に頼らず、専用の `--warmup` 経路を使います。
...
`lib/` 内の import、`guis/*.py`、`Main.py` の `PRELOAD_LIBS`、`README.md`、
`README.ja.md` を確認します。
...
`pybabel update`、`#~ obsolete entry` の削除を行います。`msgstr` は
自動入力しません。古い obsolete 翻訳を参照用に残したい場合は、実行前に
Git commit またはバックアップを作成してください。
...
通常のコンパイルやランチャーの `.po` フォールバックで翻訳として扱われないため、
配布前に残さないでください。
