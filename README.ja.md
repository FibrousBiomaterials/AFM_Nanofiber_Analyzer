...
各ファイバーは**中心線**に沿って計測します。どの画素が 1 本のファイバーかは
スケルトンが決め、その各点を、ファイバーの高さ断面が最大値の半分になる 2 点の
中点へ移します（`docs/algorithms.ja.md` の §4.2 を参照）。輪郭長はこの線の
ユークリッド長で、高さもこの線に沿って読みます。形式 1.1 より前に解析した
バンドルは、再解析するまでスケルトントラックに沿って計測し、そのことをログに
出します。

`straightness` は、ファイバーの両端間のユークリッド距離を輪郭長で割った値です。
直線状のファイバーは、線のわずかな横方向ノイズを除いて 1.0 になります（合成の
直線で 0.9994）。形式 1.1 より古いバンドルでは、両端点を結ぶ離散化された直線の
長さを輪郭長で割り、どちらも同じ補正済みチェーンコード尺度で測るため、直線状の
スケルトントラックがちょうど 1.0 になります。

`curvature` はファイバーに沿った平均回転率で、「曲率窓」欄で指定する弧長にわたって
測ります。この窓は省略できません。数画素の範囲では、回転角はファイバーの形では
なく線自身の横方向ノイズで決まるためです。半径 80〜400 nm の合成円弧（画素 2 nm）
に対し、中心線の推定は 100 nm の窓で 1/R の 2% 以内、50 nm で 3% 以内でしたが、
20 nm の窓は 400 nm の円弧を 1.9 倍曲がっていると読みました。ステップが直交か
斜めしか無いスケルトントラックでは、20 nm の窓で 1.3〜5.3 倍ずれました。窓を
広げると推定は滑らかになりますが、窓より短いファイバーが全て落ちるため、除外
された本数をログに出します。
...
`length` があるのは、点を等しく数えると 2 つの偏りが入るためです。点は骨格
画素ごとに 1 つあり、骨格は直交ステップと斜めステップを混在させるため、1 点が
代表する線の長さは 1 枚の画像内でも約 1.41 倍異なります。画像間では走査範囲に応じてピクセル
...
キンクは、個別表示と全体像のどちらでも、ファイバーの線の上に水色の塗りつぶした
点で描きます。ファイバーの端から見かけ幅 1.5 本分より近い折れは、代わりに灰色の
中空の円で描きます。片側にある繊維が短すぎるため、キンク規則はそれを測っても
判定せず、どのキンク数にも数えません（`docs/algorithms.ja.md` の §4.4 を参照）。
...
| `dp` | `(2, M)` | 形式 1.0 のキンク規則がキンクを判定した折れ線の頂点。形式 1.1 以降の規則は線を分解しないため、空（`M` = 0）で書きます。 |
| `ka` | `(N,)` | キンク内角（ラジアン）。`kp` の各列に 1 つ対応。探針が頂点を丸める半幅の外側で折れの両脇に読んだ 2 本の腕のなす角です（`docs/algorithms.ja.md` の §4.3 を参照）。 |
| `ke` | `(N,)` | 各キンクを判定した超過回転（ラジアン）。`kp` の各列に 1 つ対応。±0.75 W の窓の中の回転のうち、両脇での繊維自身の曲率で説明できない分です。形式 1.1 から書きます。 |
| `up` | `(2, K)` | キンク規則が繊維の端から見かけ幅 1.5 本分以内で測ったため判定しなかった折れ。座標規約は下記参照。GUI04 は灰色の中空の円で描き、数には入れません。形式 1.1 から書きます。 |
...
座標規約: `kp[0]`・`dp[0]`・`up[0]` が x（列）インデックス、`kp[1]`・`dp[1]`・
`up[1]` が y（行）インデックスです。いずれも `calibrated` 画像上の 0 始まりのピクセル
...
| キー | 内容 |
|---|---|
| `params` | 解析パラメータ辞書。`<input_stem>_param.json` と同一内容。`kinkangle_deg` は読み戻して使用します（後述）。`kink_decompose_px` は形式 1.0 のバンドルでのみ読み戻して使用します。それ以外のフィールドは来歴情報です。 |
| `version` | バンドル形式バージョン（現在は `"1.1"`。`"1.0"` のバンドルは再解析するまでスケルトントラックに沿って計測する）。 |
...
表示・計測されます。`kinkangle_deg` をバンドルから取ることで、1 枚の画像の中の
規則が 1 つに保たれます。`kink_decompose_px` はもう解析に影響しません。形式 1.0 の
バンドルは以前の折れ線規則で判定されているため、GUI04 がその中でフィブリルを
組み立て直すときには、記録された値を今も適用します（`docs/algorithms.ja.md` の
§4.6 を参照）。しきい値が記録される前に書かれたバンドルには当該項目が存在せず、
その場合はその実行が使った値へフォールバックします。
