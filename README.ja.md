...
（µm、両軸へ適用）をバンドルへ記録し、装置ヘッダから読み取った値を上書きします。
省略時はヘッダのスキャンサイズがあればそれを使い、無ければスキャンサイズは
保存されません。スキャンサイズはファイル単位で扱われるため、異なるサイズの
スキャンが混在するフォルダでも、バッチ共通の 1 値に頼らず正しく処理できます。
