...
計測量セレクタでは `height`、`contour length`、`kink angle`、
`kink density`（輪郭長 1 µm あたりのキンク数）を選べます。高さは細線化画素
位置の補正済み画像から取得し、残る 3 つは `cli.py measure` と Fiber Tracker が
使うのと同じファイバー単位の計測から算出するため、走査範囲が記録された
バンドルが必要です。
