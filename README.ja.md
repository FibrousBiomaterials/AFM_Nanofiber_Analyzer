...
再現性のあるファイバー単位の計測値（長さ、高さ、分岐、キンク角）を得ることを
...
| `guis/GUI02_PlotProfiler.py` | Plot Profiler | 生データ、補正済みデータ、またはバンドル化された AFM 高さデータを読み込み、選択した線分に沿った高さプロファイルを対話的に抽出します。プロファイル距離を再現可能にするため、スケールは記録値（`.b2z`）またはヘッダ（テキスト/CSV）の走査範囲で既定化します。 |
...
高さの値はナノメートルとして解釈されます。物理スキャンサイズはヘッダに記録が
あれば装置ヘッダ（島津の `SizeX` / `SizeY`、または Gwyddion の `# Width` /
`# Height`）から読み取り、バンドルへ空間較正として保存します。ヘッダにスキャン
サイズが無い入力（ヘッダを持たない Bruker NanoScope エクスポート等）は、代わりに
GUI／CLI からスキャンサイズを与えます。サンプルスキャンは
`testdata_tunicateCNF/`（島津）と `Bruker_testdata/`（代表 1 件の Bruker
NanoScope エクスポート）に同梱されています。
...
[Gwyddion](http://gwyddion.net/) でテキストに変換すれば対応できます。ファイルを
開き、**File > Save As** で **「Export Text」**（プレーンテキストのデータ行列）を
選びます。本ローダーはその `.txt` を直接読み込みます。Gwyddion は行列を SI 単位
（メートル）で、走査サイズと値の単位を記録した小さなコメントヘッダ付きで出力し、
本ローダーは両方を自動正規化します（高さを nm、サイズを µm へ）。ヘッダのキー語は
Gwyddion の UI 言語で書かれますが、英語（`# Width`/`# Height`/`# Value units`）と
日本語（`# 幅`/`# 高さ`/`# 値の単位`）の両方を認識します（翻訳語ではなく値の構造で
判定するため）。スキャンサイズを保持するため、エクスポート時は情報ヘッダを有効の
ままにしてください。無効の場合は GUI／CLI で設定します。Bruker の `.spm` バイナリ
も同様に NanoScope からテキスト出力すれば、上記の 1 列形式で読み込めます。
...
| `input_format` | 検出したテキストレイアウト（`kind`、`skiprows`、`n_cols`、`encoding`）。 |
...
- 入力テキスト/CSV の高さ値はナノメートルとして解釈されます。他の単位で出力した
  データは、読み込み前に変換してください。
- 1 列形式（Bruker NanoScope）のエクスポートは、データを正方形 `(s, s)` 画像へ
  reshape するため、高さ値の総数が平方数である必要があります。
- 長さの計測は、スキャンサイズが得られる場合にのみ物理的に意味を持ちます。
  スキャンサイズは装置ヘッダ（島津の `SizeX` / `SizeY`）から読み取るか、
  GUI／CLI から与えます。いずれも無い場合、距離はピクセル単位で報告されます。
...
| 役割 | 名前 |
|---|---|
| 解析アルゴリズムと AFM ドメイン手法 | Shingo Kiyoto, Keita Mayumi, Tomoki Ito, Kayoko Kobayashi |
| GUI とアプリケーションパッケージング | Shingo Kiyoto |
