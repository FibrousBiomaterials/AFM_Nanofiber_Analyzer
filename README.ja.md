...
あれば装置ヘッダ（島津の `SizeX` / `SizeY`）から読み取り、バンドルへ空間較正
として保存します。ヘッダにスキャンサイズが無い入力（ヘッダを持たない Bruker
NanoScope エクスポート等）は、代わりに GUI／CLI からスキャンサイズを与えます。
サンプルスキャンは `testdata_tunicateCNF/`（島津）と `Bruker_testdata/`
（代表 1 件の Bruker NanoScope エクスポート）に同梱されています。
