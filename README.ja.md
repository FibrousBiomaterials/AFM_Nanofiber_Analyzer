...
任意のファイバー連結モード（トグルと設定ウインドウ）では、交差・分岐で分断された
...
それを知らせるものが何もありませんでした。保存されるのは決定とそのしきい値であり、連結結果
そのものではありません。計測時にその値からコネクタを実行し直すため、ファイルが
横にあるバンドルと食い違うことがありません。このファイルを持つバンドルを読み込む
とその設定が採用され、ボタンはファイルが表示内容と一致しなくなったときにだけ
有効になります。除外サイドカーと異なり、連結を OFF にして保存した場合はファイルを
削除せず `enabled: false` を記録するため、調整済みのしきい値が保持されます。
...
後からかけることができます。実行にはファイバー連結が OFF である必要があります。
孤立は追跡された状態の断片に対して定義され、連結は交差を越えてファイバーを
ネットワークへつなぐため、そのファイバーは孤立でなくなるからです。密な
...
| `lib/connect_selection.py` | バンドル横の `<stem>_connect.json` に保存するファイバー連結設定。GUI04 が書き出し、`lib/measure.py` が適用する。保存するのは決定とそのしきい値だけで、連結結果そのものは保存しない。コネクタは決定的であり、結果を保存すればバンドルと食い違い得るキャッシュになるため。 |
