...
フォルダを選択すると、各走査のフィードバック不良も検査します。フィードバックが
外れると走査線がまるごと変位し、解析の複数の段が画像全体の統計からしきい値を
決めているため、グリッチ帯 1 つが視野の残り全体で検出を抑え込むことがあります。
縞ノイズ率の列に影響を受けた走査線の割合を表示し、グリッチのない走査線範囲を
ログに列挙し、該当する走査線を Original プレビューに網掛けします。この検査は
診断のみで、パイプラインの計算結果は一切変えません。

走査の一部しか使えない場合は、走査線範囲の列に入力した範囲だけを解析できます
（`473-696,709-842`）。「縞ノイズで分割」で検査結果から埋められ、「分割を解除」で元の 1 項目へ戻せます。各範囲はそれ
ぞれ独立した項目と `.b2z` になり、1 枚の画像として解析されます。これが要点で、
各段は配列全体の統計からしきい値を決めるため、乱れた走査線を除くと残りの走査線
が何と比較されるかが変わります。記録される走査範囲は範囲に合わせて縮小されるので、
切り出しの有無で同じ繊維の計測長は変わりません。CLI では
`cli.py process --rows` が同等です。
...
または `fiber csv`（Fiber Tracker が出力した `_fibers.csv` のフォルダ）を選べます。
CSV 経路があるのは、自動フィルターでは密なネットワークをキュレーションできない
ためです。そこでは追跡されたほぼ全てのファイバーが交差に接するため、ゴミや走査線
アーティファクトを落とせるのは Fiber Tracker 上での目視除外だけであり、その出力は
まさに確認を通過した母集団になります。バンドル入力の場合は、チェックを外さない
限り各バンドルの `_excluded.json` サイドカーから同じ除外が適用され、フォルダごとに
何本外れたかがログに出ます。「連結を適用」はファイバー連結について同じことを行い
ます。Fiber Tracker が `_connect.json` サイドカーを書き出したバンドルは、骨格断片
ではなくそこで表示されていた 1 本のフィブリルとして計測され、各フォルダで何個の
バンドルがその設定を持っていたかがログに出ます。連結設定はバンドル単位のため、
フォルダを 1 バンドルずつキュレーションできます。この個数表示が、部分的にしか
キュレーションされていないフォルダを可視化します。

計測量セレクタでは `height`、`contour length`、`straightness`、`curvature`、
`kink angle`、`kink density`（輪郭長 1 µm あたりのキンク数）を選べます。いずれも
`cli.py measure` と Fiber Tracker が使うのと同じファイバー単位の計測から算出します。
除外と連結は追跡済みファイバーに作用するものであり、骨格マスク上では表現できない
ためです。高さはピクセルサイズに依存しないため、走査範囲が未記録のバンドルでも
計測できる唯一の計測量です。残りは追跡ステップを nm へ変換するために走査範囲を
必要とします。

ここにある量はすべて個々のファイバーの形状を表します。被覆度の類の量は意図的に
用意していません。試料は分散液を基板に滴下・乾燥させたものであり、走査面をどれだけ
密に覆っているかは希釈率・滴下量・乾燥した液滴内のどこを走査したかで決まり、材料の
性質ではないためです。

`straightness` は、ファイバーの両端点を結ぶ離散化された直線の長さを輪郭長で
割った値で、どちらも同じ補正済みチェーンコード尺度で測ります。この尺度は離散化
された直線経路をユークリッド弦より約 5% 短く報告するため、生の弦/輪郭長では
直線状のファイバーが 1.055 になってしまいます。基準線を同じ方法で測ることで、
直線状のファイバーがちょうど 1.0 になります。

`curvature` はファイバーに沿った平均回転率で、「曲率窓」欄で指定する弧長にわたって
測ります。この窓は省略できません。骨格のステップは直交か斜めしか無いため、画素
スケールでは回転角が 45 度の倍数に量子化され、推定値は一定のノイズ下限になります。
既知半径の円弧に対し、20 nm の窓は真の曲率によらず 19.4 rad/µm を返しましたが、
100 nm 以上では 1/R を約 15% の範囲で再現しました。窓を広げると推定は滑らかに
なりますが、窓より短いファイバーが全て落ちるため、除外された本数をログに出します。

集計単位セレクタは 1 標本を何と数えるかを決めます。`pixel`（骨格画素 1 点）、
`length`（骨格画素 1 点を、それが代表する輪郭長で重み付け）、`kink`
（キンク 1 点）、`fiber`（ファイバー 1 本。高さとキンク角はそのファイバー内の
中央値）、`image`（画像 1 枚。その画像のファイバー値の中央値）です。この
区別は適用するだけでなく報告もします。骨格画素をまとめた分布は独立観測では
なく、長いファイバーほど多くの画素を出すため、統計量テーブルと図中の注釈は
標本数を「標本数・ファイバー数・画像数」に分けて表示します。

`length` があるのは、骨格画素を等しく数えると 2 つの偏りが入るためです。
1 枚の画像内では骨格が直交ステップと斜めステップを混在させ、補正済み
チェーンコード長は約 1.41 倍異なります。画像間では走査範囲に応じてピクセル
サイズが変わるため、細かくサンプリングされた画像ほどファイバー 1 µm あたりの
点数が多くなり、集約分布を支配します。各点をそのステップ長で重み付けすると
両方が取り除かれ、分布がスケール不変になります。すなわち「サンプル点のうちの
割合」ではなく「観測した輪郭長のうちの割合」を表します。標本量は輪郭長 (µm)
として報告され、生値 CSV には `weight_nm` 列が付くため、出力ファイルから同じ
分布を再現できます。

図セレクタでは `histogram`・`ECDF`・`box` を選べます。ECDF は全群を 1 つの軸に
置き、ビン幅の選択が無いため、ビンを変えて差を見せたり消したりできません。箱ひげは
テーブルが報告するのと同じ四分位数（重み付きを含む）から各群を要約します。重み付き
四分位数は Matplotlib の boxplot 自体では計算できません。

統計量テーブルの下の比較テーブルは、全てのグループ対を両側 Mann-Whitney U 検定と
2 標本 Kolmogorov-Smirnov 検定で検定し、全ペアに Holm 補正を適用したうえで、標本数
とともに増大しない効果量として Cliff's delta を報告します。検定を提供するのは
`fiber` と `image` 単位に限ります。骨格画素・長さ重み付けの点・1 本のファイバーから
出た複数のキンクは独立観測ではなく、それらに対する p 値は試料が異なるかどうかでは
なく、画像をどれだけ細かくサンプリングしたかを測ってしまうためです。

各グループは平均・標準偏差・最頻値に加えて中央値と四分位範囲を報告します。
ファイバー形態の分布は右に裾を引き、最頻値はヒストグラムのビン幅によって
動くためです。描画範囲外の標本は黙って捨てずログに割合を出し、要約統計量は
常に全標本を対象とします。標本が 8 個未満のグループには、ヒストグラムの形状を
読み取れない旨を表示します。これは `image` では通常の状況で（グループの標本数は
走査枚数そのもの）、その場合はテーブルの中央値と四分位範囲を参照します。
...
特性を繰り返し測ったものではなく、それぞれ独立した欠陥事象です。したがって
ファイバー単位でまとめてもファイバーの性質を表さず、しかも 1 本あたりのキンクは
0 個か 1 個であることが多いため、統計としても成立しません。個々の角度は CSV の
`kink_angles_deg` 列に全て出力されます。
...
交差・分岐で分断された骨格断片は、計測前に 1 本のフィブリルへ再結合できます。
「自動連結」は続きとなる断片を探索し、見つかった結果を採用します。「手動で連結…」
は、選択した 1 本のファイバーについて、その端の近くにある候補を、距離・角度・
自動探索の条件を満たすかどうかとともに提示します。自動の条件が見送った「続き」も、
画像を見て判断すれば連結できます。「連結を解除」はフィブリルを構成断片へ戻し、
「連結を取消」は直前の連結判断を 1 つ戻します（自動連結 1 回分も 1 操作です）。
モード切替ではなくボタンなのは、これらが生むのが保持される結果だからです。
「自動連結」を再度押すと現在の連結は破棄され（実行前に確認します）、しきい値を
編集しても次に押すまで何も変わりません。連結と高さフィルターは「連結してから
フィルター」の順序で合成され、両方が効いているときは高さフィルターが連結済み
フィブリルを自身の高さプロファイル（橋渡し部を含む）で切り出すため、両者は排他
ではありません。
...
ウインドウが 56 本の断片を表示しているのに GUI03 が 29 本のフィブリルを集計し、
それを知らせるものが何もありませんでした。保存されるのは、どの断片がどの順序と
向きで繋がっているかであり、それを生んだ設定でも、連結が自動か手動かでもありま
せん。結果を保存することが、除外が連結を*作り出す*ことを防ぎます。`A-B-C` が
記録された状態で `B` を除外すると `A` と `C` は別々のまま残ります。探索をやり直せ
ば `C` が `A` にとって唯一残った候補となるため両者は連結されてしまいます。断片は
トラック上のアンカー画素で指し、ファイルは構築元となった骨格の指紋を持つため、
Image Preprocessor でバンドルを再解析した場合、計測は既に存在しない骨格に対して
黙って進むのではなくファイルを拒否します。このファイルを持つバンドルを読み込む
とその連結が復元され、保存ボタンはファイルが表示内容と一致しなくなったときにだけ
有効になります。除外サイドカーと異なり、何も連結していない状態で保存した場合は
ファイルを削除せず空の結果を記録するため、調整済みのしきい値が保持されます。

ファイバーは手動でも除外できます。選択して全体像で確認し、「選択を除外」を押すと、
一覧・全体像・CSV 出力の対象から外れます。「直前を取消」で最後の除外を取り消せ、
繰り返し押すと追加した逆順にさかのぼれます。変更は表示側には即座に反映されますが、
サイドカーへの書き出しは「除外・連結を保存」を押したときだけ行われます。これらのファイルは
GUI03 が集計対象を決めるための解析入力であり、誤クリックが独りでに書き換えては
ならないためです。未保存のままデータセットを切り替える・フォルダを変える・
ウィンドウを閉じる場合は、保存・破棄・中止を選ぶ確認が出るため、黙って決まる
ことはありません。密なネットワークを解析可能にするのは
この機能です。そこではほぼ全てのファイバーが交差に接するため、ゴミや走査線
アーティファクトを本物のフィブリルから分ける自動フィルターが存在しません。除外は
バンドル横の `<stem>_excluded.json` に保存されるため、セッションを越えて残り、
データと共に持ち運べ、アプリ外での監査・編集もできます。各除外は行番号ではなく
除外対象のトラック上のアンカー画素を記録します。ファイバー連結の ON/OFF でリストの
採番は変わりますが、画素は意味を保つためです。「除外設定...」を開くと一覧が表示
され、任意の 1 件または全件を解除できます。

「非孤立を除外」は「選択を除外」の一括版です。全長を計測できなかったファイバー
（`lib.measure.isolated_fiber_flags`）を 1 回の操作でまとめて除外します。対象は、
経路上のどこかで他のファイバーに接しているファイバー、走査範囲の外へ出ていく
ファイバー、連結器から見て続きのあるファイバーです。他のファイバーとの交差部で
切断されたファイバーの長さは「短い」のではなく「切り詰められている」ため、その
断片を母集団に残すと長さ統計が過小側に偏ります。除外されたファイバーは手で選んだ
除外とまったく同じように記録され、メモには `not isolated` が入ります。そのため
「直前を取消」1 回で全部が戻り、「除外設定...」では各件がなぜ除外されたのかを
確認できます。

表示フィルターではなく除外であるため、判定は追跡された状態のファイバーに対して
一度だけ下されます。したがって、孤立ファイバーの母集団に対して高さフィルターを
後からかけることができます。実行には何も連結されていない必要があります。孤立は
追跡された状態の断片に対して定義され、連結は交差を越えてファイバーをネットワーク
へつなぐため、そのファイバーは孤立でなくなるからです。両者は断片化に対する代替の
答え（完全なものだけを測るか、完全にするか）であるため、ボタンは自ら連結を解体せず
前提条件を伝えます。密な
ネットワーク像では大半のファイバーが交差に達するため、残る本数が少ないのは
想定どおりの結果であり、検出失敗ではありません。なお、ここで判定するのは
ファイバーのトポロジーであって、対象が繊維かどうかではありません。スキャン
ライン由来のアーティファクトはどこにも接触しないため残ります。それは GUI01 の
縞ノイズ選別が担当する範囲です。

AFM 全体像には Plot Profiler と同じパン/ズームのツールバーに加え、リセット
ボタン、選択ファイバーへズームするボタン、ファイバー番号・枠の表示切り替えが
あります。密なスキャン像を読めるようにするのがズームの目的です。全体表示では
ファイバー番号が重なって判読できないうえ、番号の描画は再描画コストの最大要因
でもあります。番号を OFF にするか、拡大する（表示範囲内の番号だけを描画します）
と、移動操作の反応が目に見えて良くなります。vmin/vmax・各フィルター・表示モード
の変更では現在の表示範囲を維持し、軸目盛単位やスキャンサイズの変更では軸が
張り替わるため全体表示に戻ります。ズーム中に表示範囲外のファイバーを選ぶと、
拡大率を変えずにその位置へ移動します。全体像の画像保存では、画面に見えている
範囲がそのまま出力されます。

個別表示ウインドウの拡大像は、追跡範囲の周囲に余白を付けて表示し（既定 10 px、
0 を指定すると従来どおり追跡範囲ぴったりの切り出しに戻ります）、その追跡範囲を
破線枠で示します。追跡範囲の外接矩形はファイバーに密着しているため、余白なしでは
ファイバーの両端が必ず枠に接し、追跡が本当の端点で終わったのか、隣接ファイバー
との交差部で切れたのかを像から判別できません。これは上記 2 つのフィルターが扱う
のと同じ区別です。余白は同じ補正画像を表示用に切り直すだけで、計測値は一切
変わりません。
...
[`docs/algorithms.ja.md`](docs/algorithms.ja.md) は、前処理 4 段階（背景補正・
二値化・細線化・キンク検出）が実際に何をしているか、各パラメータが何を変える
か、そしてなぜその判断がなされたかを、ソースへの参照付きで説明します。本
ソフトウェアが報告する数値を、再現するだけでなく自分で説明する必要があるとき
に参照してください。英語版は [`docs/algorithms.md`](docs/algorithms.md) です。

この文書はコードとの一致が機械的に保たれています。
`tests/test_algorithm_docs.py` が、文書の挙げる全シンボルの実在、全解析
パラメータの記載を検証し、さらにアルゴリズム 4 モジュールを指紋化するため、
計算内容が変わると説明を見直すまでテストスイートが失敗します。

### API ドキュメント

`lib/` 以下のモジュールは Sphinx でドキュメント化されており、docstring から
API リファレンスが生成されます。上記のアルゴリズム解説と同じサイトに収録され
ます。公開版は `.github/workflows/docs.yml` が `main` からビルドし、
GitHub Pages で配信します。
...
| キー | 内容 |
|---|---|
| `params` | 解析パラメータ辞書。`<input_stem>_param.json` と同一内容。`kinkangle_deg` と `kink_decompose_px` は読み戻して使用します（後述）。それ以外のフィールドは来歴情報です。 |
...
`params` 内のキンクしきい値は、単なる来歴情報ではなく読み戻して使用します。
GUI04 は、バンドルに含まれないトラックを組み立てるたびにキンク点を再計算します
（断片から再結合したフィブリル、高さフィルターで切り出したサブファイバー）。
それらのファイバーは、キンクをバンドルからそのまま受け取ったファイバーと並べて
表示・計測されます。`kinkangle_deg` と `kink_decompose_px` をバンドルから取ることで、
1 枚の画像の中の規則が 1 つに保たれます。しきい値が記録される前に書かれた
バンドルには当該項目が存在せず、その場合はその実行が使った値へフォールバック
します。
...
| `lib/connect_selection.py` | バンドル横の `<stem>_connect.json` に保存するファイバー連結結果。GUI04 が書き出し、`lib/measure.py` が適用する。保存するのは、どの断片がどの順序と向きで繋がっているか。断片はアンカー画素で指し、見つけた骨格の指紋を添えるため、再解析されたバンドルは、既に持っていない骨格を記述したプランで計測されるのではなく拒否される。連結が自動か手動かは意図的に記録しない。 |
| `lib/fiber.py` | ファイバー形状、高さプロファイル、キンクインデックス、端点インデックスを保持する不変 `Fiber` dataclass。 |
| `lib/fiber_connector.py` | 交差・分岐で分断された骨格断片を 1 本のフィブリルへ再結合する。探索（`plan_from_auto_connect`）と構築（`build_connected_fibers`）に分割し、探索が到達した決定を保存して、再探索なしに組み立て直せるようにしている。`connection_candidates` は GUI04 の手動連結を支える。 |
| `lib/group_compare.py` | GUI03 用の群間比較。全ペアに Holm 補正を適用した Mann-Whitney U 検定と 2 標本 KS 検定、および標本数とともに増大しない効果量として Cliff's delta を提供する。 |
| `lib/fiber_selection.py` | バンドル横の `<stem>_excluded.json` に保存する手動ファイバー除外。GUI04 が書き出し、`lib/measure.py` が適用する。除外はリストのインデックスではなく除外対象のトラック上のアンカー画素であり、ファイバー連結による採番変更後も意味を保つ。 |
...
| `lib/measure.py` | `.b2z` バンドルに対する GUI 非依存のファイバー計測。`measure_bundle`、ファイバーごとの `FiberStats`、`collect_fiber_stats`（複数バンドルにまたがるファイバー単位統計）、`collect_skeleton_height_profiles`（追跡済み高さと、各点が代表する輪郭長）、`isolated_fiber_flags`（他のファイバーに接していないファイバーの判定）、スケルトン画素高さの収集、および GUI03/GUI04 と `cli.py` が共有する CSV 書き出し。 |
