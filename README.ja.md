...
これとは別に、孤立ファイバーフィルターは一覧・全体像・CSV 出力の対象を、経路上の
どこでも他のファイバーに接していないファイバーに限定します
（`lib.measure.isolated_fiber_flags`）。他のファイバーとの交差部で切断された
ファイバーの長さは「短い」のではなく「切り詰められている」ため、その断片を母集団
に残すと長さ統計が過小側に偏ります。このフィルターは計測済みのファイバーから
選び出すだけで、再解析は行いません。

孤立ファイバーフィルターとファイバー連結は排他で、一方を ON にすると他方は OFF
になります。両者は同じ問題に正反対の方針で答えます。連結は交差を越えてファイバー
を再構築し、本フィルターは交差に達したファイバーを信頼しません。同時に有効に
しても目的を損なうだけです。連結は孤立ファイバーをネットワークへつなぐため、
その繊維は孤立ではなくなります。密なネットワーク像では大半のファイバーが交差に
達するため、残る本数が少ないのは想定どおりの結果であり、検出失敗ではありません。
なお、このフィルターが判定するのはファイバーのトポロジーであって、対象が繊維か
どうかではありません。スキャンライン由来のアーティファクトはどこにも接触しない
ため通過します。それは GUI01 の縞ノイズ選別が担当する範囲です。
