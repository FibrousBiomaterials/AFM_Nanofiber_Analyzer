...
- **(c) Fiber Height Histogram** はユーザー定義の試料グループ間で、高さ・輪郭長・
  キンク角・キンク密度の分布を、骨格画素単位・ファイバー単位・画像単位で比較し、
  グループごとの統計量を表示します。
...
`kink`（キンク 1 点）、`fiber`（ファイバー 1 本。高さとキンク角はそのファイバー
内の中央値）、`image`（画像 1 枚。その画像のファイバー値の中央値）です。この
...
動くためです。描画範囲外の標本は黙って捨てずログに件数を出し、要約統計量は
常に全標本を対象とします。
...
| `lib/measure.py` | `.b2z` バンドルに対する GUI 非依存のファイバー計測。`measure_bundle`、ファイバーごとの `FiberStats`、`collect_fiber_stats`（複数バンドルにまたがるファイバー単位統計）、`isolated_fiber_flags`（他のファイバーに接していないファイバーの判定）、スケルトン画素高さの収集、および GUI03/GUI04 と `cli.py` が共有する CSV 書き出し。 |
