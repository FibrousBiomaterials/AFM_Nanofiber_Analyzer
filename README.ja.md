...
`kink angle`、`kink density`（輪郭長 1 µm あたりのキンク数）を選べます。高さは
細線化画素位置の補正済み画像から取得し、残りは `cli.py measure` と Fiber Tracker
が使うのと同じファイバー単位の計測から算出するため、走査範囲が記録されたバンドルが
必要です。
...
サイドカーへの書き出しは「除外を保存」を押したときだけ行われます。このファイルは
