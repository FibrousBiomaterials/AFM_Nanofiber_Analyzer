...
4 種類の `bg_method` のどれが試料に合うかは事前には自明でないため、
`lib/bg_quality.py` が結果を採点します。この指標は、どのスキャンを ML モデルの
学習に使う価値があるかを選別するために存在します。ML タスクの学習対象は古典
パイプラインの出力なので、背景補正が失敗したスキャンは誤ったラベルを供給します。
`bgquality` は手元にあるバンドルについて指標を報告します:
...
測定する欠陥は 3 つです。いずれも他の 2 つが正常を示す状況で実際に発生する
ことが確認されており、それぞれ対処法が異なります。

**繊維の脇のハロー**。繊維近傍で背景推定が誤っていることによります。`halo_nm`
は `global_threshold`（既定 0.3 nm）と比較して読みます。これを超える残留は、
実際の繊維に並走する偽ファイバーとして二値化されます。

| 指標 | 意味 |
|---|---|
| `halo_nm` | 残った高さ（正）または過剰減算（負）。探索して何も見つからなければちょうど `0.0`。 |
| `halo_asymmetry_nm` | 片側が溝、反対側が尾根。この反対称アーティファクトは両側をまとめる統計量では 0 に相殺されるため、専用の数値が必要。 |
| `halo_position_px` | 欠陥がピークを取る繊維からの距離。繊維に密着したハローはマスク境界を、より外側のものは平滑化窓を示唆する。 |
| `halo_wide_nm` | 断面より広いハロー。断面自身の参照レベルごと引きずるため、これが無ければ「ハロー無し」と読まれてしまう。 |

**縞残差**。背景モデルが除去しきれなかったライン間オフセットによります。
`row_residual_nm` と `col_residual_nm` は横縞・縦縞の強さで、これらも
`global_threshold` と比較し、かつ**対で**読みます。比が大きければ縞の向きが
分かり、したがってどの `spline1d_axis` が効くかが決まります。

**マスク足跡**。補正器の繊維マスクを過剰に膨張させ、実在する基板構造まで背景
プールから除外してしまうことによります。モデルがその構造を再現できなくなり、
繊維が背景に対して浮きます。`mask_footprint_nm` は膨張半径に残る段差です。
繊維マスクを作らない `tophat` では未設定になります。

ハロー検出は、スケルトン上の各点で局所繊維方向に垂直な断面を標本化し、
二値化マスクを一切参照しません。繊維に並走するハローはその繊維の二値化成分へ
併合されてしまい、マスクを基準にした測定からは見えなくなるためです。

ハローは、断面の微分から特定した極値——繊維の頂点、そして曲面が下降を止める
転回点——で測ります。経験則で置いた帯を平均する方式は実際の対象と部分的にしか
重ならず、最も重要な大きなハローほど過小評価するためです。高さと併せて繊維から
の距離も報告します。

制約を 1 つ把握しておいてください。繊維に十分近い*正の*ハローは斜面の減衰を
緩やかにするだけで、プロファイルは単調なままとなり、ハローと繊維裾を分離する
特徴が現れません。この場合 `halo_nm` はちょうど `0.0` を返します。これが正直な
答えですが、その系統的なケースは `mask_footprint_nm` と `halo_wide_nm` が
カバーするので、併せて確認してください。同じ距離にある溝は必ずプロファイルを
転回させるため検出されます。

各断面がどこまで届くかは固定画素数ではありません。繊維幅は断面自身から
（ハローの影響を受けないよう半値で）測り、標本化長はその幅とマスク膨張半径に
応じて伸びます。画像が小さすぎてそこまで届かない場合は、標本化していない対象を
報告する代わりに `warnings` へ記載します。

総合スコアは意図的に用意していません。これらを 1 つの数値へまとめるには
物理的根拠のない重みが必要になるからです。個別に読み、1 つの値を絶対的な
誤差量として解釈するのではなく、同一画像上で方式を比較してください。入力が
`bg_method` だけ異なる同一ファイルの実行群である場合は、`--union-mask` を
付けると全入力が同一の画素集合で採点されます。

### 2 つの処理条件の比較

2 つの設定のどちらを採るかは、同じ入力に両方を走らせて比較して決めます。
`bgcompare` はファイル名でバンドルを対にし、両者を同一の画素集合で採点し、
ファイルごとに画像パネルを出力します:
...
結論が自信をもって述べられる経路だからです。

- **繊維の母集団を分割**します（実繊維と、それより短い断片。境界は
  `--min-fiber-um`、既定 1 µm）。悪い設定は通常、実繊維を劣化させるのではなく
  1 nm 未満の短い誤検出を*追加*するため、全体をまとめた中央値だけが崩れ、実繊維は
  無傷のままになります。同梱スキャンの 1 件では、`mask_dilation` 3 → 12 は
  合算すると高さ −85% の崩壊に見えますが、1 µm 以上の繊維では −1.4% で、偽の
  断片が 160 個増えていただけでした。
- **補正済み・二値化・細線化の各画像**を固定表示レンジで並べて描画します。
  繊維数 2 対 8 という数値は「繊維が失われた」と読めますが、画像を見れば両方の
  繊維が無傷だと分かります。
...
| `lib/measure.py` | `.b2z` バンドルに対する GUI 非依存のファイバー計測。`measure_bundle`、ファイバーごとの `FiberStats`、`isolated_fiber_flags`（他のファイバーに接していないファイバーの判定）、スケルトン画素高さの収集、および GUI03/GUI04 と `cli.py` が共有する CSV 書き出し。 |
