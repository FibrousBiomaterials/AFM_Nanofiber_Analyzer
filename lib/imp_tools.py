...
def convert_track_to_distance(xtrack: np.ndarray,
    ...
    ----------
    xtrack
        X-coordinate (column index) array in tracing order.
        追跡順の X 座標（列インデックス）配列。
    ytrack
        Y-coordinate (row index) array in tracing order.
        追跡順の Y 座標（行インデックス）配列。
    pixel_step_size
        Physical size of one pixel along the X (column) axis.
        X（列）軸方向の 1 画素の物理サイズ。
    y_pixel_step_size
        Physical size of one pixel along the Y (row) axis. ``None`` reuses
        ``pixel_step_size``, i.e. an isotropic (square-pixel) scale.
        Y（行）軸方向の 1 画素の物理サイズ。``None`` のときは
        ``pixel_step_size`` を流用する（等方＝正方ピクセルのスケール）。
